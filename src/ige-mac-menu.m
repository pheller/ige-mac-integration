...
window_focus(GtkWindow *window, GdkEventFocus *event, CarbonMenu *menu) {
    CocoaTarget *target = g_object_get_qdata (G_OBJECT (window), 
					  cocoa_target_quark);
    OSStatus err = SetRootMenu(menu->menu);
    current_focussed_window = GTK_WIDGET (window);
    if (err) {
...
ige_mac_menu_set_menu_bar (GtkMenuShell *menu_shell) {
    ...
    CFStringRef cfstr = CFStringCreateMutable(NULL, 256);
    CocoaTarget *target;

    g_return_if_fail (GTK_IS_MENU_SHELL (menu_shell));
    current_focussed_window = parent;
    if (cocoa_target_quark == 0)
	cocoa_target_quark = g_quark_from_static_string ("CocoaTarget");
    target = g_object_get_qdata (G_OBJECT (parent), cocoa_target_quark);
 ...
 * menu.
 * 
 * We can create a phony GtkMenu and reparent items from other menus
 * onto it, then call sync_menu_shell on it with the app_menu as the
 * carbon argument.
 */
...
ige_mac_menu_add_app_menu_group (void) {
    ...
    gint index = app_menu_group_last_index(NULL);
    app_menu_insert_separator(index);
